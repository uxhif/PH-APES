...
class ShapeNet(BaseDataset):
    ...
    def load_data_list(self):
        ...
        ph_prefix = self.data_prefix.get('ph_path', None)
        for pcd_name, cls_label_name, seg_label_name, ph_name in zip(sorted(os.listdir(pcd_prefix)),
                                                            sorted(os.listdir(cls_label_prefix)),
                                                            sorted(os.listdir(seg_label_prefix)),
                                                            sorted(os.listdir(ph_prefix))):
            data_list.append(dict(classes=self.metainfo['classes'],
                                  ...
                                  ph_path=os.path.join(ph_prefix, ph_name)))
        return data_list
